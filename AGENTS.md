...
- The app is organized into **6 categories** (常绿乔木 / 落叶乔木 / 灌木 / 宿根 / 匍匐攀缘 / 水生植物), derived from the `category` field. The original Excel "design layers" (设计层) are collapsed into these categories; the `designLayer` field was removed as redundant.
- Keep the first version intentionally minimal: browse the 6 categories, search by name, filter by category, plant detail + images.
